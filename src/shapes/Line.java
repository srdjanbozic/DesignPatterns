...
public class Line extends Shape {
	...
	private double getDistanceFromPoint(int x, int y) {
		double numerator = Math
				.abs((endPoint.getY() - startPoint.getY()) * x - (endPoint.getX() - startPoint.getX()) * y
						+ endPoint.getX() * startPoint.getY() - endPoint.getY() * startPoint.getX());

		double denominator = Math.sqrt(
				Math.pow(endPoint.getY() - startPoint.getY(), 2) + Math.pow(endPoint.getX() - startPoint.getX(), 2));
